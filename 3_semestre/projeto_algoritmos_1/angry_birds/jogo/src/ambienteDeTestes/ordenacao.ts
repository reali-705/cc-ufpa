...
import { Passaro } from "../classes/passaro.ts";
import { Porco } from "../classes/porco.ts";


function testandoOrdenacao() {
    const niveis = importarDados<LevelConfig[]>('testes.json')!;
    if (!niveis) return console.error("Nenhum nivel carregado");
    ...
    for (let i = 0; i < niveis.length; i++) {
        ...
        let passaroBuscado: Passaro;
        let porcoBuscado: Porco;
        let start: number;
        let end: number;
